...
<div class="container">
    <div class="card">
        ...
        <div class="error-box" style="background:#f0fff4;border-color:#b7ebc8;color:#1a7f3c">
            &#10003; Email yuborildi — pochta qutingizni tekshiring
        </div>
        <% } else if ("activated".equals(msg)) { %>
        <div class="error-box" style="background:#f0fff4;border-color:#b7ebc8;color:#1a7f3c">
            &#10003; Hisob faollashtirildi — endi kiring
        </div>
        <% } else if ("invalid_token".equals(msg)) { %>
        <div class="error-box">&#9888; Tasdiqlash havolasi yaroqsiz yoki muddati o'tgan</div>
        <% } %>
        <% if (error != null) { %>
        ...
        %>
        <form method="post" action="${pageContext.request.contextPath}/login<%= next != null ? "?next=" + next : "" %>">
            <div class="field">
                <label>Email</label>
                <input type="text" name="email" placeholder="email kiriting" required>
            </div>
            <div class="field">
                <label>PAROL</label>
        ...
        <p class="foot">Hisobingiz yo'qmi? <a href="${pageContext.request.contextPath}register">Ro'yxatdan o'ting</a>
        </p>
    </div>
</div>
</body>
